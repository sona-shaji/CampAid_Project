const SupplyRequest = require("../Models/RequestSupplyModel");
const mongoose = require("mongoose");

// Submit a new supply request
const submitRequest = async (req, res) => {
  try {
    const { campId, officerId, itemId, quantityRequested } = req.body;

    if (!campId || !officerId || !itemId || !quantityRequested) {
      return res.status(400).json({ error: "All fields are required!" });
    }

    const newRequest = new SupplyRequest({ campId, officerId, itemId, quantityRequested });
...
const updateRequestStatus = async (req, res) => {
  try {
    ...
    const existingRequest = await SupplyRequest.findById(requestId);
    if (!existingRequest) {
      return res.status(404).json({ error: "Request not found!" });
    }

    const updatedRequest = await SupplyRequest.findByIdAndUpdate(requestId, updateData, { new: true });
